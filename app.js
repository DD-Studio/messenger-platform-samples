...
const ALCHEMY_API_KEY = (process.env.ALCHEMY_API_KEY) ?
  ...
  config.get('alchemyApiKey');

if (!(APP_SECRET && VALIDATION_TOKEN && PAGE_ACCESS_TOKEN && ALCHEMY_API_KEY)) {
  console.error("Missing config values");
  process.exit(1);
}

var alchemyLanguage = watson.alchemy_language({ api_key: ALCHEMY_API_KEY });
...
function receivedMessage(event) {
  ...
  if (messageText) {
    ...
    switch (messageText) {
      ...
      default:
        sendEmotionMessage(senderID, messageText);
    }
...
/*
 * Send an emotion analyized message using the Send API.
 *
 */
 function sendEmotionMessage(recipientId, messageText) {
   var params = { text: messageText };
...
function createBarChartUrl(emotionData) {
  ...
  bar.addAxisLabels('x', [
    'Anger' + emotionData["anger"].toFixed(2),
    'Disgust' + emotionData["disgust"].toFixed(2),
    'Fear' + emotionData["fear"].toFixed(2),
    'Joy' + emotionData["joy"].toFixed(2),
    'Sadness' + emotionData["sadness"].toFixed(2)
  ]);
